...
func_hist_plot <- function(data, predictors, folder_path = "figures") {
...
<div style="display: flex; flex-wrap: wrap; justify-content: space-around;">
  <div class="plot"><img src="figures/PG_value_hist.png" alt="PG Value Histogram"/></div>
  <div class="plot"><img src="figures/PK_hist.png" alt="PK Histogram"/></div>
  <div class="plot"><img src="figures/informed_hist.png" alt="Informed Histogram"/></div>
  <div class="plot"><img src="figures/agree_value_hist.png" alt="Agree Value Histogram"/></div>
  <div class="plot"><img src="figures/trustworthy_value_hist.png" alt="Trustworthy Value Histogram"/></div>
  <div class="plot"><img src="figures/believable_value_hist.png" alt="Believable Value Histogram"/></div>
  <div class="plot"><img src="figures/factual_value_hist.png" alt="Factual Value Histogram"/></div>
  <div class="plot"><img src="figures/accurate_value_hist.png" alt="Accurate Value Histogram"/></div>
  <div class="plot"><img src="figures/election_reg_hist.png" alt="Election Regulation Histogram"/></div>
...
# Check residuals for normality
qqnorm(resid(ainform_pk)) 
qqline(resid(ainform_pk))

# variance of residuals
df_residuals <- data.frame(fitted = fitted(ainform_pk), residuals = resid(ainform_pk))
...
ran_ef <- ranef(ainform_pk, condVar = TRUE)
...
recall_df <- imprint_df %>%
  select(id, advert, version, CSC, BBA, SFI, CBB) %>%
  mutate(
    CSC = as.integer(CSC),
    BBA = as.integer(BBA),
    SFI = as.integer(SFI),
...
name_recall <- glm(recall ~ version, data = recall_df, family = binomial())

```

#### Table

Two tables are shown as tab_model does not show the original beta coeffecients (needed to interpret the direction of the effect).

```{r, echo=FALSE}

#p-values

sum_recall_table <- tab_model(name_recall, show.est = TRUE, show.aic = TRUE) 

sum_recall_table

#to additionally show beta-coefficients
...
recall_table <- summarise_model(name_recall)
...
recall_table <- column_names(recall_table)
...
recall_table <- row_names(recall_table)

kable(recall_table, format = "html", digits = 2, caption = "Recall of campaigner name (coeffecient shown)") %>%
...
preds_recall <- ggpredict(name_recall, terms = "version [all]")

ggplot(preds_recall, aes(x = x, y = predicted, group = 1)) +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0.1) +
  geom_point() +
  ylim(0, 1) +
  labs(title = "Predicted Probability of campaigner name recall by version of material viewed",
       x = "Imprint viewed", y = "Probability of recall") +
  theme_minimal()
...
One issue with the previous model, although simple, is that it does not account for nested data. In this case, the recall may vary between adverts based on factors independent from the presence or absence of a digital imprint, such as the aesthetic design that may have impacted recall independently of the version viewed.

```{r}

#random intercepts introduced for advert, varying by the version viewed

random_recall <- glmer(recall ~ version + (1|advert), 
               data = recall_df, 
               family = binomial(link = "logit"))

#checking convergence, the outcome suggests all estimator optimisers can successfully estimate this model

allFit(random_recall)

```

```{r, echo=FALSE}

tab_model(random_recall, show.aic = TRUE)
...
As the predictor was categorical, many of the assumptions required for a logistic regression (linearity of the logit) naturally hold. The following assesses the model fit, and compares the simpler model with the model with random effects:

- Chi-squared: A significant result suggests the model is a better fit for the data than a null model.
- AIC: can be used to compare models, the smaller the number the better the relative fit.

The following chi-squared test indicates that including the 'imprint viewed' as a binary predictor significantly improves the model fit compared to the null model.

```{r, echo=FALSE}

#calculating chi-squared statistic for logistic regression model

null_model <- glm(recall ~ 1, data = name_recall$model, family = binomial)
chi_sq_stat <- null_model$deviance - name_recall$deviance
df <- name_recall$rank - null_model$rank
...
The following AIC scores show that the random effect model is a slightly better fit for the data compared to the simpler logit model.

```{r, echo=FALSE}

AIC(name_recall, random_recall)
...
res_random <- qqmath(residuals(random_recall, type = "pearson"))
...
save_lattice_plot(res_random, "recall_residual_random.png")
...
<div style="display: flex; flex-wrap: wrap; justify-content: space-around;">
  <div class="plot"><img src="figures/recall_residual_simple.png" alt="Simple residual plot"/></div>
  <div class="plot"><img src="figures/recall_residual_random.png" alt="Random residual plot"/></div>
...
# Check residuals for normality
qqnorm(resid(inform_pi)) 
qqline(resid(inform_pi))

# Variance of residuals
df_residuals <- data.frame(fitted = fitted(inform_pi), residuals = resid(inform_pi))
...
ran_ef <- ranef(inform_pi, condVar = TRUE)
...
# Check residuals for normality
qqnorm(resid(inform_training)) 
qqline(resid(inform_training))

# Residual variance
df_residuals <- data.frame(fitted = fitted(inform_training), residuals = resid(inform_training))
...
ran_ef <- ranef(inform_training, condVar = TRUE)
