...
# Digital imprint
mixed_model_TOST(inform_int, term = "version1", sesoi_d = 0.10)

# Interaction 
mixed_model_TOST(inform_int, term = "version1:Training.condition1", sesoi_d = 0.10)
...
pk_int <- lmer(PK ~ version*Training.condition + (1 | id) + (1|advert), data = imprint_df, control = lmerControl(optimizer = "bobyqa"))
...
# Digital imprint
mixed_model_TOST(pk_int, term = "version1", sesoi_d = 0.10)

# Interaction 
mixed_model_TOST(pk_int, term = "version1:Training.condition1", sesoi_d = 0.10)
...
### Perceieved credibility

```{r H3, echo=FALSE, fig.align='center'}

cred_int <- lmer(credibility ~ version*Training.condition + (1 | id) + (1|advert), data = imprint_df)
...
# Digital imprint
mixed_model_TOST(cred_int, term = "version1", sesoi_d = 0.10)

# Interaction 
mixed_model_TOST(cred_int, term = "version1:Training.condition1", sesoi_d = 0.10)
...
# Welch's t-test (unequal variances) — NA-safe
welch <- t.test(election_reg ~ Training.condition,
                data = betweenmeasure_df,
                var.equal = FALSE,    # Welch (default, but explicit)
                conf.level = 0.95,
                na.action = na.omit)
welch
