...
# ERRORS FOR STIMULI-CLUSTER CODE

# Cook's D with influence.ME

infl_pk <- influence(pk_mod, group = "advert")  # cluster-level influence object
cd   <- cooks.distance(infl_pk)             # Cook’s D per cluster (id)
...
pk_cook_l2 <- ggplot(cd_df, aes(x = advert, y = cooksD)) +
  ...
  labs(
    ...
    x = "Cluster index (experimental stimuli)",
...
re <- ranef(pk_mod, condVar = TRUE)$advert
postVar <- attr(ranef(pk_mod, condVar = TRUE)[[1]], "postVar")
z_b <- as.numeric(re[[1]] / sqrt(postVar[1, 1, ]))

# Q–Q plot
pk_qq_l2 <- ggplot(data.frame(z_b = z_b), aes(sample = z_b)) +
  stat_qq(shape = 16) +
...
pk_l2_diagnostics <- (pk_qq_l2 / pk_cook_l2) +
  plot_annotation(
    title = "Level-2 Diagnostics for Persuasion Knowledge Model",
    subtitle = "A: Normality of Random Effects (stimuli)   |   B: Cluster Influence (Cook’s D)",
...
ggsave(
  filename = "assumptions/pk_L2_diagnostics_advert.png",
  plot = pk_l2_diagnostics,
...
```{r source identification level two assumptions}
