...
factor_loadings <- factor_loadings %>% dplyr::select(Item, everything())
...
randomisation <- glm(Training.condition ~ partyID + age + gender + education + Ethnicity.simplified, family = binomial(link = "logit"), data = training_df)
...
ggplot(pred, aes(x = x, y = predicted, color = response.level)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(title = "Predicted Probabilities for PPI by Version",
...
#### Cumalitive Link Multi-level Model

The previous assumption charts show severe violation regarding the normality of the residuals, due to the skew of the variable. This section will use various methods in an attempt to create a model where assumptions are more closely met, to ensure valid interpretations.
...
polr_model <- polr(PPI ~ version + Training.condition, data = imprint_df, method = "logistic")

brant(polr_model)

pred <- ggeffect(clmm_model, terms = "version")  # Get predicted probabilities
ggplot(pred, aes(x = x, y = predicted, color = response.level)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(title = "Predicted Probabilities for PPI by Version",
...
confint(clmm_model, parm = "version")
...
mixed(Credibility ~ Training.condition + version + Training.condition:version + (1|id) + (1|advert), data = imprint_df, control = lmerControl(optimizer = "bobyqa"), method = 'LRT')
