...
author:
-   name: Prof Katharine Dommett, Prof Tom Stafford, Dr Junyan Zhu, Hazel Gordon
output: 
  html_document:
    theme: "flatly"
    toc: true
...
Above are the R packages used for analysis and visualisation. Please ensure you have these installed if you wish to knit the script. The package 'MASS' is used in the final stage of the script. This is loaded later to avoid package conflicts with tidyverse/dplyr.
...
*Model suitability considerations*

However, it is also recognised a variety of random effect structures would be theoretically feasible. To choose the most appropriate model out of the alternatives, model fit criterion are used to assess if adding model complexity sacrifices valid estimation through increasing the possibility of a type ... error (REF). 

In this case, the final chosen model included random intercepts for participant ID and advert. As can be seen in the model variation comparisons, including random slopes worsened the fit of the model when evaluated using AIC values. This strongly implies adding model complexity in the form of random slopes does not aid a valid estimation of effects in this case, likely due to the small effects of digital imprint version.

- Outcome: PG_value, numerical 1-7
- Predictor: version, binary factor
- Random effects: id and advert (intercepts)
...
imprint_df %>% 
  ...
  geom_jitter(alpha = 0.1) +
  geom_smooth(mapping = aes(group = advert), method = "lm", se = FALSE, fullrange = TRUE) +
  labs(y = "Percieved Persuasion Knowledge",
    colour = "Advert", x = "Version") +
  theme_minimal()
...
imprint_df <- imprint_df %>%
  rowwise() %>%
  mutate(credible = mean(c(trustworthy_value, believable_value, accurate_value)))
...
outcomes <- c("credible", "factual_value")
...
tab_credible <- tab_model(model_credible, pred.labels = c("Intercept", "Training", "Digital Imprint included", "Agreement", "Training*imprint"), dv.labels = "Perceived credibility")

tab_credible
#summary(model_credible)

tab_fact <- tab_model(model_factual_value, pred.labels = c("Intercept", "Training", "Digital Imprint included", "Agreement", "Training*imprint"), dv.labels = "Perceived fact versus opinion")
...
preds_credible <- ggpredict(model_credible, terms = c("Training.condition", "version"))
preds_factual <- ggpredict(model_factual_value, terms = c("Training.condition", "version"))
...
# Check residuals for normality
qqnorm(resid(model_credible)) 
qqline(resid(model_credible))

# Residual variance
df_residuals <- data.frame(fitted = fitted(model_credible), residuals = resid(model_credible))
...
ran_ef <- ranef(model_credible, condVar = TRUE)
