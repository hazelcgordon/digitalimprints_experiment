...
### A note about the hypotheses and analysis in final paper
...
### Hypothesis 2a and 2b, outcome: recall of names
...
### Hypotheses 3a and 3b: outcome: perceived informedness
...
### Hypothesis 4a and 4b: outcome: trustworthiness
...
tab_trust <- tab_model(model_credible, pred.labels = c("Intercept", "Training", "Digital Imprint included", "Agreement", "Training*imprint"), dv.labels = "Perceived credibility")
...
regulation_control <- lm(election_reg ~ Training.condition + external_efficacy + internal_efficacy + SM_use + partyID + age + gender + education + Ethnicity.simplified + political_interest + political_trust + political_cynicism + political_mistrust + EPE_mean, data = training_df)

best_model <- step(regulation_control)

tab_model(best_model, show.se = TRUE)

```

#### Table

The top table shows the original outcome. The bottom table shows a robust regression model, weights generated using an M estimator with the 'rlm' function in the MASS package (loaded at this point in the script). A robustness check is included due to the violation of assumptions seen in the original model.
...
factor_analysis <- cfa(cfa, data = imprint_df)

summary(factor_analysis, fit.measures = TRUE, standardized = TRUE)
