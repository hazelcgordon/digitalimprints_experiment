...
ainform_pg2 <- lmer(PG_value ~ version + Training.condition + (1 | id) + (1|advert), data = imprint_df)

#random slopes for version and advert

ainform_pg3 <- lmer(PG_value ~ version + Training.condition + (1 | id) + (1 + version|advert), data = imprint_df)

# fixed effect interaction for version and advert

ainform_pg4 <- lmer(PG_value ~ version*advert + Training.condition + (1 | id), data = imprint_df)

# basic model

ainform_pg5 <- lmer(PG_value ~ version + advert + Training.condition + (1 | id), data = imprint_df)
...
#summary(ainform_pk2)
#allFit(ainform_pk2)
#anova(ainform_pk3, ainform_pk4)
#AIC(ainform_pk2, ainform_pk6)

# checking against a reduced model

library(afex)

mixed(PG_value ~ version + Training.condition + (1 | id) + (1|advert), data = imprint_df, control = lmerControl(optimizer = "bobyqa"), method = 'LRT')

mixed(PG_value ~ version + Training.condition + (1 | id) + (1+version|advert), data = imprint_df, control = lmerControl(optimizer = "bobyqa"), method = 'LRT')

mixed(PG_value ~ version*advert + Training.condition + (1 | id), data = imprint_df, control = lmerControl(optimizer = "bobyqa"), method = 'LRT')

mixed(PG_value ~ version + advert + Training.condition + (1 | id), data = imprint_df, control = lmerControl(optimizer = "bobyqa"), method = 'LRT')

#comparing model fit

AIC(ainform_pg2, ainform_pg3, ainform_pg4, ainform_pg5)
...
imprint_df %>% 
  ggplot(mapping = aes(x = factor(version), y = PK, colour = factor(advert))) +
  geom_point() +
  geom_jitter(alpha = 0.1) +
  geom_smooth(mapping = aes(group = advert), method = "lm", se = FALSE, fullrange = TRUE) +
  labs(y = "Percieved Persuasive Intent",
...
inform_pg_tab2 <- tab_model(ainform_pg2, 
                           pred.labels = c("Intercept", "Digital Imprint 
                                           included"),
                           dv.labels = "Perceived Political Nature")
...
ainform_pk2 <- lmer(PK ~ version + Training.condition + (1 | id) + (1|advert), data = imprint_df, control = lmerControl(optimizer = "bobyqa"))

#random slopes for version and advert

ainform_pk3 <- lmer(PK ~ version + Training.condition + (1 | id) + (1 + version|advert), data = imprint_df)

# fixed effect interaction for version and advert

ainform_pk4 <- lmer(PK ~ version*advert + Training.condition + (1 | id), data = imprint_df)

# basic model

ainform_pk5 <- lmer(PK ~ version + advert + Training.condition + (1 | id), data = imprint_df)
...
mixed(PK ~ version + Training.condition + (1 | id) + (1+version|advert), data = imprint_df, control = lmerControl(optimizer = "bobyqa"), method = 'LRT')

mixed(PK ~ version*advert + Training.condition + (1 | id), data = imprint_df, control = lmerControl(optimizer = "bobyqa"), method = 'LRT')
...
AIC(ainform_pk2, ainform_pk3, ainform_pk4, ainform_pk5)
BIC(ainform_pk3, ainform_pk5)
...
inform_pk_tab2 <- tab_model(ainform_pk2, 
                           pred.labels = c("Intercept", "Digital Imprint 
                                           included"),
                           dv.labels = "Persuasion knowledge")
inform_pk_tab2

tab_model(ainform_pk3)
...
#below is the final model, choson due to it's low AIC.
...
null <- glmer(recall ~ (1|id), 
               data = recall_df, 
               family = binomial(link = "logit"))

imprint_model <- glmer(recall ~ version + (1|id), 
               data = recall_df, 
               family = binomial(link = "logit"))
...
material_model <- glmer(recall ~ version + Training.condition + 
                           advert + (1|id), 
               data = recall_df, 
               family = binomial(link = "logit"))
...
#THIS CODE IS NOW INCORRECT AS BASED ON WRONG MODEL

#p-values

sum_recall_table <- tab_model(name_recall, show.aic = TRUE) 
...
initial <- glmer(recall ~ version + Training.condition + (1|id) +
                   (1|advert), data = recall_df, family = binomial(link = "logit"))

#fit is singular error message for 'maximal' model

maximal <- glmer(recall ~ version + Training.condition + (1|id) + 
                   (version|advert), 
               data = recall_df, 
               family = binomial(link = "logit"))

interaction <- glmer(recall ~ version*advert + Training.condition + 
                     (1|id), 
                     data = recall_df, 
                     family = binomial(link = "logit"), 
                     control = glmerControl(optimizer = "bobyqa", 
...
material_fixed <- glmer(recall ~ version + Training.condition + 
                          advert + (1|id), 
               data = recall_df, 
               family = binomial(link = "logit"))

AIC(initial, interaction, material_fixed)
